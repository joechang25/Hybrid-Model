...
def validate(resize_valid_ds, valid_ds, resize_model, pretrained_model, loss_fn, resize_loss_fn, args):
    ...
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    correct_56 = AverageMeter(':6.2f')
    total_56 = AverageMeter(':6.2f')
    correct_112 = AverageMeter(':6.2f')
    total_112 = AverageMeter(':6.2f')
    correct_224 = AverageMeter(':6.2f')
    total_224 = AverageMeter(':6.2f')

    progress = ProgressMeter(
        num_batch,
        [batch_time, resize_time, inference_time, total_time, None, losses, top1, top5, correct_56, total_56, correct_112, total_112, correct_224, total_224],
        prefix='Test: ',
        fmt=args.print_format)
    ...
    for i in range(data_size[args.dataset]):
        ...
        resize_num[int(resize_output/112)] += 1
        # measure accuracy and record loss
        ...
        num = int(resize_output/112)
        if acc1 == 100.0:
            resize_cor[num] += 1
        if resize_num[0] != 0:
            correct_56.update(resize_cor[0] / resize_num[0] * 100)
        total_56.update(resize_num[0] / data_size[args.dataset] * 100)
        if resize_num[1] != 0:
            correct_112.update(resize_cor[1] / resize_num[1] * 100)
        total_112.update(resize_num[1] / data_size[args.dataset] * 100)
        if resize_num[2] != 0:
            correct_224.update(resize_cor[2]/ resize_num[2] * 100)
        total_224.update(resize_num[2] / data_size[args.dataset] * 100)
...
class ProgressMeter():
    ...
    def display_tf(self, batch):
        batch_time, resize_time, inf_time, total_time, _, losses, top1, top5, cor_56, total_56, cor_112, total_112, cor_224, total_224 = self.meters
        ...
        if batch + 1 == self.num_batches:
            ...
            s = self.prefix + self.batch_fmtstr.format(batch) + \
                    ...
                    ' - 56_correct: {:2.3f}'.format(cor_56.val) + \
                    ' - 56_total: {:2.3f}'.format(total_56.val) + \
                    ' - 112_correct: {:2.3f}'.format(cor_112.val) + \
                    ' - 112_total: {:2.3f}'.format(total_112.val) + \
                    ' - 224_correct: {:2.3f}'.format(cor_224.val) + \
                    ' - 224_total: {:2.3f}'.format(total_224.val) + \
                ' - time: {}'.format(total) + \
