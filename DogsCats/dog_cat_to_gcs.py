...
CLASS_INDEX = 'dog_cat_labels.json'
...
def convert_to_tf_records(
  ...
  # Create labels, which have the same mappings for keras pretrain models.
  with open(CLASS_INDEX) as f:
    maps = json.load(f)
    labels = {bytes(maps[label][0], 'utf-8'): int(label) for label in maps}
...
def run(raw_data_dir: str,
  ...
  convert_to_tf_records(raw_data_dir=raw_data_dir, local_scratch_dir=local_scratch_dir)
