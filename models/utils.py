...
def get_normalize_fn(arch):
    return {
        ...
        'customnet': models.resnet.preprocess_input
    }[arch]
...
def get_resumed_model(args):
    ...
    return tf.keras.models.load_model(args.resume, custom_objects=custom_ob, compile=False)
    #return tf.keras.models.load_model(args.resume, compile=False)
'''
...
def get_resized_model(class_size, args):
    print('  => using resized model')
    model_class = models.model_class[args.arch]
    input_shape = (args.image_size, args.image_size, 3)
    check_input_shape(args)
    #weights = args.weights
    check_input_shape(args)
    return model_class(weights=None,
                       input_shape=input_shape,
                       include_top=True,
                       classes=class_size)
...
def add_model_top(model, class_size, args):
    ...
    if args.mixed:
        ...
        x = tf.keras.layers.Dense(class_size, activation='softmax',dtype='float32')(x)
    return x


def get_custom_model(class_size, args):
    print("Get custom model")
    model_class = models.model_class[args.arch]
    input_shape = (args.image_size, args.image_size, 3)
    ...
    output = add_model_top(model, class_size, args)
...
def get_model(class_size, args):
    print("  => creating model '{}'".format(args.arch))

    #if args.resume:
    #    model = get_resumed_model(args)
    if args.pretrained:
        model = get_resized_model(class_size, args)
    else:
        model = get_custom_model(class_size, args)
